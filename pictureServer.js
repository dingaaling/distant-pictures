...
parser.on('data', function(data) {
  ...
  if (data=='light') {
    ...
    console.log('making a rotated picture at'+ imageName); // Second, the name is logged to the console.

    //The picture is taken and saved to the `public/`` folder
    NodeWebcam.capture('public/'+imageName, opts, function( err, data ) {
    
    gm(imagePath+".jpg").rotate('black', 180).write(imagePath+'Rotate.jpg', function (err) {
    if (err) console.log(err);
    });
    io.emit('newPicture',(imageName+'Rotate.jpg')); ///Lastly, the new name is send to the client web browser.
